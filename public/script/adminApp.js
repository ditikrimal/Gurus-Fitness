...
var AddNoticeButton = $("#add-notice-button");
var NewNoticeSection = $("#new-notice-section");
var NewNoticeBox = $("#new-notice-box");
var CloseNoticeSection = $("#notice-form-cancel-btn");

AddNoticeButton.click(function () {
    NewNoticeSection.addClass("active");
    NewNoticeBox.addClass("active");
});

CloseNoticeSection.click(function () {
    NewNoticeSection.removeClass("active");
    NewNoticeBox.removeClass("active");
});

var AddPlanButton = $("#add-plan-button");
var NewPlanSection = $("#new-plan-section");
var NewPlanBox = $("#new-plan-box");
var ClosePlanSection = $("#plan-form-cancel-btn");

AddPlanButton.click(function () {
    NewPlanSection.addClass("active");
    NewPlanBox.addClass("active");
});

ClosePlanSection.click(function () {
    NewPlanSection.removeClass("active");
    NewPlanBox.removeClass("active");
});
...
/* create event ajax code end*/

/* create notice ajax code*/

$(document).ready(function () {
    $("#newNoticeForm").submit(function (event) {
        event.preventDefault();
        // Prevent the form from submitting via the browser
        var formData = new FormData(document.getElementById("newNoticeForm"));
        formData.append("_token", $('meta[name="csrf-token"]').attr("content"));

        $.ajax({
            url: "/admin/notices/create-notice", // The route to handle the request
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function () {
                ...
                // Display the success message
                flashMessage.innerHTML = "Notice Added Successfully";
            ...
            error: function (response) {
                ...
                if (response.status == 422) {
                    var errors = response.responseJSON.errors;
                    if (errors.notice_title) {
                        $("#RepMsg3").fadeIn();

                        $("#RepMsg3").html(
                            '<i class="fa-solid fa-triangle-exclamation" style="color:red; font-size:11px;"></i> ' +
                                errors.notice_title[0]
                        );
                    }
                }
            },
...
/* create notice ajax code end*/

/* create plans and price ajax code */

$(document).ready(function () {
    $("#newPlanForm").submit(function (event) {
        event.preventDefault();
        // Prevent the form from submitting via the browser
        var formData = new FormData(document.getElementById("newPlanForm"));
        formData.append("_token", $('meta[name="csrf-token"]').attr("content"));

        $.ajax({
            url: "/admin/plans/create-plan", // The route to handle the request
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function () {
                ...
                // Display the success message
                flashMessage.innerHTML = "Plan Added Successfully";
            ...
            error: function (response) {
                ...
                if (response.status == 422) {
                    ...
                    if (errors.plan_prices) {
                        $("#RepMsg4").fadeIn();

                        $("#RepMsg4").html(
                            '<i class="fa-solid fa-triangle-exclamation" style="color:red; font-size:11px;"></i> ' +
                                errors.plan_prices[0]
                        );
                    }
                    if (errors.plan_features) {
                        $("#RepMsg4").fadeIn();

                        $("#RepMsg4").html(
                            '<i class="fa-solid fa-triangle-exclamation" style="color:red; font-size:11px;"></i> ' +
                                errors.plan_features[0]
                        );
                    }
                }
            },
...
//shake maximum length textare notice content

const textarea = document.querySelector(".notice_textarea");
textarea.addEventListener("input", () => {
    if (textarea.value.length >= 255) {
        textarea.classList.add("maxLength");
        textarea.animate(
            [
        ...
        RepMsg3.innerHTML = "Maximum 255 characters allowed";
    } else {
        textarea.classList.remove("maxLength");
